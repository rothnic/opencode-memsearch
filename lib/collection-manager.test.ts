...
const isCollectionSuccess = <T>(
	result: CollectionResult<T>,
): result is { ok: true; data: T } => {
	return result.ok === true;
};

const isCollectionError = <T>(
	result: CollectionResult<T>,
): result is { ok: false; error: CollectionManagerError } => {
...
describe("DEFAULT_METADATA_FIELDS", () => {
	...
	test("includes source_session field as string", () => {
		const sourceField = DEFAULT_METADATA_FIELDS.find(
			(f) => f.name === "source_session",
		);
	...
	test("includes technology field as string", () => {
		const techField = DEFAULT_METADATA_FIELDS.find(
			(f) => f.name === "technology",
		);
...
describe("CollectionManagerError", () => {
	test("creates error with correct properties", () => {
		const error = new CollectionManagerError("cli_not_found", "CLI not found", {
			retryable: true,
		});
	...
	test("defaults retryable to false", () => {
		const error = new CollectionManagerError(
...
describe("CollectionManager.fromConfig", () => {
	...
	test("creates instance with custom metadata fields", () => {
		...
		const manager = CollectionManager.fromConfig({
			defaultMetadataFields: customFields,
		});
		expect(manager).toBeInstanceOf(CollectionManager);
	});
...
describe("CollectionManager.collectionExists", () => {
	test("throws on invalid collection name", async () => {
		const manager = CollectionManager.fromConfig();
		await expect(manager.collectionExists("!invalid")).rejects.toThrow(
			CollectionManagerError,
		);
	});
	...
	test("throws on empty name", async () => {
		const manager = CollectionManager.fromConfig();
		await expect(manager.collectionExists("")).rejects.toThrow(
			CollectionManagerError,
		);
	});
	...
	test("returns true when collection exists", async () => {
		const mockShell = createMockShell({
			...
			'sh -c "memsearch stats --collection test_collection"': {
				stdout: "Documents: 10",
				stderr: "",
				exitCode: 0,
			},
	...
	test("returns false when collection does not exist", async () => {
		const mockShell = createMockShell({
			...
			'sh -c "memsearch stats --collection nonexistent"': {
				stdout: "",
	...
	test("throws on CLI error other than not found", async () => {
		const mockShell = createMockShell({
			...
			'sh -c "memsearch stats --collection test"': {
				stdout: "",
		...
		const manager = CollectionManager.fromConfig({ shell: mockShell as never });
		await expect(manager.collectionExists("test")).rejects.toThrow(
			CollectionManagerError,
		);
	});
});
...
describe("CollectionManager.getCollectionInfo", () => {
	test("returns info with exists true when collection exists", async () => {
		const mockShell = createMockShell({
			...
			'sh -c "memsearch stats --collection my_collection"': {
				stdout: '{"documentCount": 5, "chunkCount": 100}',
				stderr: "",
				exitCode: 0,
			},
		});
	...
	test("returns info with exists false when collection does not exist", async () => {
		const mockShell = createMockShell({
			...
			'sh -c "memsearch stats --collection missing"': {
				stdout: "",
				stderr: "collection not found",
				exitCode: 1,
			},
	...
	test("parses plain text stats output", async () => {
		const mockShell = createMockShell({
			...
			'sh -c "memsearch stats --collection text_stats"': {
				stdout: "Documents: 25\nChunks: 500",
				stderr: "",
				exitCode: 0,
			},
...
describe("CollectionManager.ensureCollection", () => {
	...
	test("creates collection when it does not exist", async () => {
		const mockShell = createMockShell({
			...
			'sh -c "memsearch stats --collection new_collection"': {
				stdout: "",
				stderr: "collection not found",
				exitCode: 1,
			},
...
describe("CollectionManager with onIndex callback", () => {
	test("calls onIndex before creating collection", async () => {
		...
		const mockShell = createMockShell({
			...
			'sh -c "memsearch stats --collection callback_test"': {
				stdout: "",
				stderr: "collection not found",
				exitCode: 1,
			},
