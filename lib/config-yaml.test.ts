...
import { loadYamlConfig, hasYamlConfig, getConfigPath, mergeWithLegacyConfig } from "./config-yaml";
...
beforeEach(async () => {
  await rm(path.join(tmpRoot, ".memsearch.yaml"), { recursive: true, force: true });
});
...
`;
      ...
      expect(result?.memoryTypes?.[0].tagLists?.[0].tags).toEqual(["architecture", "patterns"]);
...
`;
  ...
  describe("mergeWithLegacyConfig", () => {
    ...
    test("merges yaml config with legacy config", () => {
      const yamlConfig = {
        version: 1,
        memoryTypes: [{ name: "decision", collection: "memory_decision", enabled: true }],
        extraction: { defaultModel: "gpt-4", timeout: 30 },
        collections: { autoCreate: true },
      };
      const legacyConfig = { memoryDirectory: "/test", topK: 5 };

      const result = mergeWithLegacyConfig(yamlConfig as any, legacyConfig as any);

      expect((result as any).memoryDirectory).toBe("/test");
      expect((result as any).topK).toBe(5);
      expect((result as any).extraction).toEqual({ defaultModel: "gpt-4", timeout: 30 });
...
describe("loadConfig integration with memory types", () => {
  const testRoot = path.join(os.tmpdir(), "memsearch-config-integration-test-" + Date.now());
...
`
    ...
    expect((config as any).memoryTypes.some((m: any) => m.name === "decision")).toBe(true);
  });
...
`
    ...
    const decision = (config as any).memoryTypes.find((m: any) => m.name === "decision");
...
describe("migration warning from legacy opencode.json", () => {
  const migrationTestRoot = path.join(os.tmpdir(), "memsearch-migration-test-" + Date.now());
  ...
  beforeEach(async () => {
    ...
    await rm(path.join(migrationTestRoot, "opencode.json"), { recursive: true, force: true });
    await rm(path.join(migrationTestRoot, ".memsearch.yaml"), { recursive: true, force: true });
  });
  ...
  test("shows migration warning when legacy opencode.json has memsearch config but no .memsearch.yaml", async () => {
    ...
    expect(extras.migrationWarning.message).toContain("Legacy memsearch configuration detected");
  ...
  test("shows info message when both opencode.json and .memsearch.yaml exist", async () => {
    ...
    expect(extras.migrationWarning.message).toContain("YAML takes precedence");
  });
