...
import { queue, type MemoryJob } from './memory-queue';
...
const worker = new Worker(
  ...
  async (job: { id: string; name: string; data: MemoryJob }) => {
    ...
    console.log(`[memsearch] Processing ${jobData.type} for ${jobData.projectId}`);
    
    // Check if project is already being processed
    if (isProjectProcessing(jobData.projectId)) {
      console.log(`[memsearch] Project ${jobData.projectId} busy, deferring job ${job.id}`);
    ...
    try {
      ...
      if (result.success) {
        incrementCompleted();
        console.log(`[memsearch] Completed ${jobData.type} for ${jobData.projectId}`);
      } else {
        incrementFailed();
        console.error(`[memsearch] Failed ${jobData.type} for ${jobData.projectId}: ${result.error}`);
      }
