  ...
  async complete(request: LLMRequest): Promise<LLMResponse> {
    ...
    const doRequest = async (): Promise<LLMResponse> => {
      ...
      if (!choice?.message?.content) {
        throw new LLMError("invalid_response", "No content in OpenAI response", { retryable: false });
      }
...
export class OllamaClient implements LLMClient {
  ...
  constructor(config: {
    ...
    this.model = config.model;
    this.baseUrl = (
      config.baseUrl ?? process.env.OLLAMA_HOST ?? OLLAMA_DEFAULT_BASE_URL
    ).replace(/\/$/, "");
  ...
  async complete(request: LLMRequest): Promise<LLMResponse> {
    ...
    const doRequest = async (): Promise<LLMResponse> => {
      ...
      if (!json.message?.content) {
        throw new LLMError("invalid_response", "No content in Ollama response", { retryable: false });
      }
...
Max tokens: ${maxTokens}`;
    ...
    const doRequest = async (): Promise<LLMResponse> => {
      try {
        ...
        if (errorMessage.includes("timeout") || errorMessage.includes("timed out")) {
          throw new LLMError("timeout", `Forked session timed out: ${errorMessage}`, {
            retryable: true,
          });
        }
        ...
        if (errorMessage.includes("rate limit") || errorMessage.includes("rate_limit")) {
          throw new LLMError("rate_limit", `Forked session rate limited: ${errorMessage}`, {
            retryable: true,
          });
        }
   ...
   */
  private async callForkedSession(prompt: string): Promise<{ content: string }> {
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => {
        reject(new Error(`Forked session call timed out after ${this.timeoutMs}ms`));
      }, this.timeoutMs);
    });
    ...
    try {
      ...
      throw new Error(`Forked session call failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
}
 ...
 */
export interface OpenCodeTaskFn {
  (params: {
...
Max tokens: ${maxTokens}`;
    ...
    const doRequest = async (): Promise<LLMResponse> => {
      try {
        ...
        // Check for timeout
        if (errorMessage.includes("timeout") || errorMessage.includes("timed out")) {
          throw new LLMError("timeout", `Subagent execution timed out: ${errorMessage}`, {
            retryable: true,
          });
        }
        ...
        // Check for rate limiting
        if (errorMessage.includes("rate limit") || errorMessage.includes("rate_limit")) {
          throw new LLMError("rate_limit", `Subagent rate limited: ${errorMessage}`, {
            retryable: true,
          });
        }
        ...
        if (
          ...
          throw new LLMError("connection", `Failed to spawn subagent: ${errorMessage}`, {
            retryable: true,
          });
        }
  ...
  private async callSubagent(prompt: string): Promise<{ content: string }> {
    ...
    try {
      ...
      // Wrap other errors
      throw new Error(`Subagent call failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
}
 ...
 */
export interface CreateLLMClientOptions extends Omit<LLMClientConfig, "provider" | "model"> {
...
export function parseModelString(modelString: string): [string, string] {
  ...
  if (slashIndex === -1 || slashIndex === 0 || slashIndex === modelString.length - 1) {
    throw new LLMError(
      "unknown",
  ...
  return [modelString.substring(0, slashIndex), modelString.substring(slashIndex + 1)];
}
