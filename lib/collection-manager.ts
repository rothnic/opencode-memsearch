...
export const DEFAULT_METADATA_FIELDS: CollectionMetadataField[] = [
  { name: "tags", type: "string[]", description: "Array of tags for categorization" },
  { name: "source_session", type: "string", description: "Session ID that created this entry" },
  { name: "technology", type: "string", description: "Technology or tool associated with this entry" },
];
...
export class CollectionManager {
  ...
  private constructor(config: CollectionManagerConfig) {
    ...
    this.defaultMetadataFields = config.defaultMetadataFields ?? DEFAULT_METADATA_FIELDS;
  ...
  async collectionExists(name: string): Promise<boolean> {
    ...
    try {
      ...
      const result = await this.shell`sh -c "memsearch stats --collection ${name}"`.quiet();
  ...
  async getCollectionInfo(name: string): Promise<CollectionInfo> {
    ...
    try {
      ...
      const result = await this.shell`sh -c "memsearch stats --collection ${name}"`.throws();
  ...
  async createCollection(
    ...
    try {
      ...
      await this.shell`sh -c "echo ${placeholderContent} > ${placeholderPath}"`.throws(true);

      try {
        // Index the placeholder to create the collection
        await this.shell`sh -c "memsearch index ${placeholderPath} --collection ${name}"`.throws(true);
   ...
   */
  private generatePlaceholderContent(name: string, schema?: CollectionSchema): string {
