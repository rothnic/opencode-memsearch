...
import { ScopedWriter } from "./scoped-writer";
...
export class CompactionCapture {
  ...
  private constructor(config: CompactionCaptureConfig) {
    ...
    this.defaultMemoryType = config.compaction?.memoryType ?? config.defaultMemoryType ?? "context";
   ...
   */
  parseCompactionEvent(eventData: unknown): CompactionCaptureResult<CompactionSummary> {
    if (!this.isEnabled()) {
      return {
        ok: false,
    ...
    if (typeof eventData === "string") {
      ...
      sessionId = typeof data.sessionId === "string" ? data.sessionId : 
                  ...
                  typeof data.session === "string" ? data.session : "unknown";
      
      summary = typeof data.summary === "string" ? data.summary : 
                typeof data.content === "string" ? data.content : 
                typeof data.compacted === "string" ? data.compacted : "";
      ...
      messageCount = typeof data.messageCount === "number" ? data.messageCount :
                     typeof data.messages === "number" ? data.messages :
                     typeof data.count === "number" ? data.count : 0;

      tokenCount = typeof data.tokenCount === "number" ? data.tokenCount :
                   typeof data.tokens === "number" ? data.tokens : undefined;

      originalMessageCount = typeof data.originalMessageCount === "number" ? data.originalMessageCount :
                             typeof data.originalMessages === "number" ? data.originalMessages : undefined;
      ...
      const knownFields = ["sessionId", "session_id", "session", "summary", "content", "compacted",
                           "messageCount", "messages", "count", "tokenCount", "tokens",
                           "originalMessageCount", "originalMessages", "timestamp", "time",
                           "sessionPath", "path"];
      metadata = {};
  ...
  async onCompaction(
    ...
    if (typeof summary === "object" && summary !== null && "sessionId" in summary && "timestamp" in summary && "summary" in summary) {
  ...
  async captureSummary(
    ...
    const outputPath = this.determineOutputPath(summary, memoryType, memTypeConfig);


    // Generate content with frontmatter
    const content = this.formatSummaryContent(summary, memoryType, memTypeConfig);
    ...
    if (this.scopedWriter) {
      try {
        const validatedPath = this.scopedWriter.writeFile(outputPath, memoryType);
  ...
  private determineOutputPath(
    ...
    const filenamePattern = memTypeConfig?.output?.filenamePattern ?? "{date}_{session_id}.md";

    // Generate filename from pattern
    const date = new Date(summary.timestamp).toISOString().split("T")[0] || "unknown";
  ...
  private formatSummaryContent(
    ...
    const frontmatterFields = memTypeConfig?.output?.frontmatter ?? 
                              ["session_id", "project_path", "tags", "extracted_at"];
    ...
    if (frontmatterFields.includes("original_message_count") && summary.originalMessageCount) {
    ...
    if (memTypeConfig?.tagLists && memTypeConfig.tagLists.length > 0) {
      const defaultTags = memTypeConfig.tagLists.find(t => t.id === "default");
      if (defaultTags?.tags) {
        frontmatter.tags = [...(frontmatter.tags as string[] || []), ...defaultTags.tags];
      }
    }
