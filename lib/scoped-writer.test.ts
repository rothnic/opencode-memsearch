...
import { ScopedWriter, createScopedWriterFromConfig, MemoryTypeScope } from "./scoped-writer";
...
describe("ScopedWriter", () => {
  ...
  describe("addScope", () => {
    ...
    test("allows multiple paths per scope", () => {
      const writer = new ScopedWriter();
      writer.addScope("decision", ["/project/memory/decision", "/backup/memory/decision"]);
  ...
  describe("validatePath", () => {
    ...
    test("allows valid path within scope", () => {
      const validated = writer.validatePath("memory/decision/test.md", "decision");
    ...
    test("allows nested paths within scope", () => {
      const validated = writer.validatePath("memory/decision/2024/01/test.md", "decision");
      expect(path.isAbsolute(validated)).toBe(true);
      expect(validated).toContain("memory/decision");
    });
    ...
    test("rejects path with .. in middle", () => {
      expect(() => {
        writer.validatePath("memory/decision/subdir/../other/file.md", "decision");
      }).toThrow();
    });

    test("rejects sibling directory traversal", () => {
      expect(() => {
        writer.validatePath("memory/decision/../convention/secret.md", "decision");
      }).toThrow();
    });
  });
  ...
  describe("validateWrite", () => {
    ...
    test("returns validated path for write", () => {
      const validated = writer.validateWrite("/project/memory/decision/test.md", "decision");
  ...
  describe("validateDir", () => {
    ...
    test("returns validated directory path", () => {
      const validated = writer.validateDir("/project/memory/decision/2024", "decision");
  ...
  describe("isAllowed", () => {
    ...
    test("returns false for unconfigured type without throwing", () => {
      expect(writer.isAllowed("memory/decision/test.md", "non-existent")).toBe(false);
    });
  });
  ...
  describe("path normalization", () => {
    ...
    test("normalizes path separators", () => {
      const writer = new ScopedWriter([
        { type: "decision", allowedPaths: ["memory/decision//subdir///nested"] },
      ]);
  ...
  describe("multiple scopes", () => {
    ...
    test("validates against correct scope", () => {
      expect(writer.isAllowed("memory/decision/test.md", "decision")).toBe(true);
      expect(writer.isAllowed("memory/convention/test.md", "convention")).toBe(true);
    });

    test("cross-scope validation fails", () => {
      expect(writer.isAllowed("memory/decision/test.md", "convention")).toBe(false);
      expect(writer.isAllowed("memory/convention/test.md", "decision")).toBe(false);
    });
  });
  ...
  describe("edge cases", () => {
    test("path with encoded characters", () => {
      ...
      expect(writer.isAllowed("memory/decision/file%20with%20spaces.md", "decision")).toBe(true);
    });

    test("symlink traversal attempts", () => {
      const writer = new ScopedWriter([
        { type: "decision", allowedPaths: [path.join(testProjectRoot, "memory/decision")] },
      ]);
