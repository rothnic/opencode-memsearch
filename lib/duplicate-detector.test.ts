...
const testRoot = path.join(
	os.tmpdir(),
	"memsearch-dup-detector-test-" + Date.now(),
);
...
describe("DuplicateDetector", () => {
	...
	describe("fromConfig", () => {
		test("creates detector from DeduplicationConfig", () => {
			...
			expect(detector.getMemoryDirs()).toContain(
				path.join(testRoot, "memory", "decision"),
			);
		});
	...
	describe("findSimilar", () => {
		...
		test("sorts results by similarity descending", async () => {
			...
			const result = detector.findSimilar(
				"This is similar content that matches well",
			);
...
Some content here`,
	...
	describe("isDuplicate", () => {
		test("returns true when duplicate found", async () => {
			...
			expect(
				detector.isDuplicate("We use React for building user interfaces"),
			).toBe(true);
		});
	...
	describe("getSimilarity", () => {
		test("returns similarity for specific file", async () => {
			...
			const sim = detector.getSimilarity(
				"Some existing content about decisions",
				filePath,
			);
	...
	describe("edge cases", () => {
		...
		test("handles memory files with no frontmatter", async () => {
			await writeFile(
				path.join(memoryDir, "no-frontmatter.md"),
