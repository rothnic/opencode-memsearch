...
import { sanitizeCollectionName, resolveCollectionName } from "./memory-type-collection-resolver";
import MemoryTypeRegistry from "./memory-types";

const testRoot = path.join(os.tmpdir(), "memsearch-resolver-test-" + Date.now());
...
`
  ...
  test("uses provided prefix option", () => {
    const resolved = resolveCollectionName("alpha", undefined, { prefix: "px_" });
    expect(resolved.startsWith("px_") || resolved.startsWith("m" + "px_")).toBe(true);
  });
});
