...
import { MemoryTypeConfigSchema, type MemoryTypeConfig } from "./memory-type-config";

/** Global memory types directory following project conventions */
const GLOBAL_MEMORY_TYPES_DIR = path.join(homedir(), ".config", "opencode", "memory");
 ...
 */
function loadSingleMemoryTypeConfig(configPath: string): MemoryTypeConfig | null {
...
function scanMemoryTypeConfigs(
  ...
  try {
    ...
    console.warn(`memsearch: failed to scan ${source} memory types directory: ${baseDir}`);
  }
...
export function loadMemoryTypes(workdir: string): LoadedMemoryTypes {
  ...
  const projectResult = scanMemoryTypeConfigsWithValidation(projectDir, "project");
...
export function hasProjectMemoryTypes(workdir: string): boolean {
  ...
  try {
    ...
    return entries.some(
      (entry) => entry.isDirectory() && existsSync(path.join(projectDir, entry.name, CONFIG_FILENAME))
    );
  } catch {
    return false;
  }
...
export function hasGlobalMemoryTypes(): boolean {
  ...
  try {
    const entries = readdirSync(GLOBAL_MEMORY_TYPES_DIR, { withFileTypes: true });
    return entries.some(
      (entry) =>
        entry.isDirectory() &&
        existsSync(path.join(GLOBAL_MEMORY_TYPES_DIR, entry.name, CONFIG_FILENAME))
    );
  } catch {
    return false;
  }
