...
test("convertSessionToMarkdown includes ID, title, role, content", () => {
	...
	const md = convertSessionToMarkdown({
		metadata,
		history,
	} as SessionWithHistory);
...
test("loadIndexedState / saveIndexedState round-trip", async () => {
	...
	// ensure clean
	await rm(path.join(statePath, ".memsearch"), {
		recursive: true,
		force: true,
	});

	const state: IndexedState = {
		sessions: { ses_1: { indexedAt: 1000, source: "file" } },
		lastRun: 1000,
	};
...
test("indexSessions writes markdown and updates state for new sessions", async () => {
	...
	await rm(path.join(tmpRoot, ".memsearch", "indexed.json"), {
		force: true,
	}).catch(() => {});
