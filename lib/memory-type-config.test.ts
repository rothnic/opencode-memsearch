...
import { MemoryTypeConfigSchema, ModelNameRegex, CollectionNameRegex, MIN_PROMPT_LENGTH, MAX_PROMPT_LENGTH } from "./memory-type-config";
...
describe("memory-type-config schema", () => {
  test("accepts a valid config", () => {
    const cfg = {
      ...
      extractionPrompt: "Extract key decisions and rationale from the session content.",
      model: "openai/gpt-4",
      enabled: true,
