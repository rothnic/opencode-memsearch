...
const makeSession = (overrides?: Partial<SessionWithHistory>): SessionWithHistory => ({
  metadata: {
    id: "ses_abc123",
    title: "Test session",
    directory: "/tmp/test",
  ...
  history: [
    { ts: "2026-01-01T00:00:00Z", role: "user", content: "We should use PostgreSQL for persistence" },
    { ts: "2026-01-01T00:01:00Z", role: "assistant", content: "Good choice. I'll set up the schema." },
    { ts: "2026-01-01T00:02:00Z", role: "user", content: "Also, let's use kebab-case for all filenames" },
  ],
  ...overrides,
});

const makeMemoryType = (name = "decision", overrides?: Partial<PromptMemoryType>): PromptMemoryType => ({
  name,
...
describe("buildSystemPrompt", () => {
  ...
  test("includes description when present", () => {
    const mt = makeMemoryType("decision", { description: "Architectural choices" });
    const result = buildSystemPrompt([mt]);
  ...
  test("prefers tagLists over flat tags", () => {
    const tagLists: TagListInfo[] = [
      { id: "semantic", description: "Categorization tags", tags: ["arch", "db"], manageable: true },
      { id: "tech", description: "Technology tags", tags: ["postgresql"], manageable: false },
    ];
    const mt = makeMemoryType("decision", { tagLists, tags: ["should-not-appear"] });
    const result = buildSystemPrompt([mt]);
  ...
  test("accepts custom system base override", () => {
    ...
    const result = buildSystemPrompt([makeMemoryType()], { customSystemBase: customBase });
...
describe("buildUserPrompt", () => {
  ...
  test("includes session transcript", () => {
    ...
    expect(result).toContain("[user]: We should use PostgreSQL for persistence");
    expect(result).toContain("[assistant]: Good choice. I'll set up the schema.");
    expect(result).toContain("[user]: Also, let's use kebab-case for all filenames");
  });

  test("formats tool entries with tool annotation", () => {
    const session = makeSession({
      history: [
        { ts: "2026-01-01T00:00:00Z", role: "assistant", content: "running search", tool: "mem-search" },
      ],
    });
    const result = buildUserPrompt(session, [makeMemoryType()]);
  ...
  test("truncates long session content", () => {
    ...
    expect(result).toContain("[... session content truncated due to length ...]");
  });
  ...
  test("includes analysis instruction at end", () => {
    ...
    expect(result).toContain("Return the JSON output as specified in your system instructions.");
  });
});
...
describe("buildPromptForType", () => {
  ...
  test("passes options through", () => {
    const result = buildPromptForType(
      makeMemoryType(),
      makeSession(),
...
describe("buildPromptForTypes", () => {
  test("includes all types in system prompt", () => {
    const types = [makeMemoryType("decision"), makeMemoryType("convention"), makeMemoryType("context")];
...
describe("toPromptMemoryType", () => {
  ...
  test("applies tagLists override", () => {
    ...
    const tagLists: TagListInfo[] = [
      { id: "semantic", description: "Categories", tags: ["new-tag"], manageable: true },
    ];
...
describe("real config integration", () => {
  test("works with decision-style config", () => {
    const decision = makeMemoryType("decision", {
      ...
      tags: ["decision", "action-item", "meeting", "backend", "frontend", "infra"],
    });
    ...
    expect(systemPrompt).toContain("decision, action-item, meeting, backend, frontend, infra");
