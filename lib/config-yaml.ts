...
export const MemoryTypeOutputSchema = z.object({
  ...
  frontmatter: z.array(z.string()).default(["session_id", "project_path", "tags", "extracted_at"]),
});
...
export const MemoryTypeConfigSchema = z.object({
  ...
  output: MemoryTypeOutputSchema.optional().default({ path: "memory", filenamePattern: "{date}_{session_id}.md", frontmatter: ["session_id", "project_path", "tags", "extracted_at"] }),
  frequency: FrequencyConfigSchema.optional().default({ mode: "manual", intervalMinutes: 60, onCompact: false }),
});
...
export const CollectionConfigSchema = z.object({
  ...
  metadataFields: z.array(z.string()).default(["technologies", "tags", "session_id", "extracted_at"]),
});
...
/**
 * Load and parse .memsearch.yaml from a directory.
 * Returns null if file doesn't exist.
 */
export function loadYamlConfig(workdir: string): MemsearchYamlConfig | null {
  const configPath = path.join(workdir, ".memsearch.yaml");
...
/**
 * Check if .memsearch.yaml exists in a directory.
 */
export function hasYamlConfig(workdir: string): boolean {
  return existsSync(path.join(workdir, ".memsearch.yaml"));
}
...
export function mergeWithLegacyConfig(
  ...
  legacyConfig: ReturnType<typeof import("../config").loadConfig> extends Promise<infer R> ? R : never
): unknown {
  ...
  const merged: Record<string, unknown> = { ...legacyConfig } as Record<string, unknown>;
  ...
  if (yamlConfig.deduplication) {
    (merged as Record<string, unknown>)["deduplication"] = yamlConfig.deduplication;
  }
