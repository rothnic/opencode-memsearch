...
export type ExtractionErrorCode = "empty_content" | "invalid_memory_type" | "config_error";
...
const TECHNOLOGY_KEYWORDS: Record<string, string[]> = {
  ...
  // Backend Languages
  Python: ["python", "python3", "py ", "pip ", "pypi", "django", "flask", "fastapi", "pyenv"],
  ...
  // Cloud & DevOps
  AWS: ["aws", "amazon web services", "s3", "ec2", "lambda", "dynamodb", "rds", "sqs", "sns", "cloudformation", "sam "],
  GCP: ["gcp", "google cloud", "google cloud platform", "cloud run", "gke", "bigquery", "firestore"],
  ...
  VectorDB: ["vector db", "vector database", "pinecone", "weaviate", "chroma", "qdrant"],
 ...
 */
function calculateConfidence(occurrences: number, totalMentions: number): number {
...
export class TagExtractor {
  ...
  constructor(config: TagExtractorConfig) {
    ...
    this.customTagsSet = new Set(config.customTags?.map((t) => t.toLowerCase()) ?? []);
  }
  ...
  extractTags(content: string): TagExtractionResult {
    ...
    const tags: string[] = [];
    const totalMentions = Array.from(allDetected.values()).reduce((a, b) => a + b, 0);
  ...
  extractTagsForType(content: string, memoryType: string): TagExtractionResult {
    ...
    for (const tagList of tagLists) {
      ...
      if (tagList.description) {
        ...
        const descWords = tagList.description.split(/\s+/).filter((w) => w.length > 2);
    ...
    filteredTags.sort((a, b) => (filteredConfidence[b] || 0) - (filteredConfidence[a] || 0));
