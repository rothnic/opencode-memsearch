...
describe("TagExtractor", () => {
  describe("constructor", () => {
    ...
    test("creates instance with memory type tags", () => {
      const extractor = new TagExtractor({
        ...
        memoryTypeTags: {
          decision: [
            { id: "tech", description: "Technologies", tags: ["react", "vue"], manageable: true },
          ],
        },
      });
      expect(extractor).toBeDefined();
  ...
  describe("fromConfig", () => {
        ...
        memoryTypes: [
          {
            ...
            tagLists: [{ id: "t1", description: "tech", tags: ["react"], manageable: false }],
          },
        ],
      });
      expect(extractor).toBeDefined();
    ...
    test("handles memory types without tagLists", () => {
      const extractor = TagExtractor.fromConfig({
        memoryTypes: [{ name: "context", collection: "contexts", extractionPrompt: "extract context", enabled: true }],
      });
  ...
  describe("extractTags", () => {
    test("extracts React from content", () => {
      const extractor = new TagExtractor({ defaultTags: [] });
      const result = extractor.extractTags("We built this with React and TypeScript");
      expect(result.ok).toBe(true);
      if (result.ok) {
    ...
    test("extracts from code imports", () => {
      const extractor = new TagExtractor({ defaultTags: [] });
      const result = extractor.extractTags("import React, { useState } from 'react'");
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.tags).toContain("React");
      }
    ...
    test("extracts from file extensions", () => {
      const extractor = new TagExtractor({ defaultTags: [], enableHeuristics: true });
    ...
    test("extracts Docker from Dockerfile content", () => {
      const extractor = new TagExtractor({ defaultTags: [] });
      const result = extractor.extractTags("FROM node:18\
RUN npm install\
COPY . /app");
      expect(result.ok).toBe(true);
      if (result.ok) {
    ...
    test("extracts Kubernetes from k8s content", () => {
      const extractor = new TagExtractor({ defaultTags: [] });
      const result = extractor.extractTags("Deploy to Kubernetes using kubectl and helm");
      expect(result.ok).toBe(true);
      if (result.ok) {
    ...
    test("extracts databases", () => {
      const extractor = new TagExtractor({ defaultTags: [] });
      const result = extractor.extractTags("Connect to MongoDB using Mongoose, store in Redis");
      expect(result.ok).toBe(true);
      if (result.ok) {
    ...
    test("extracts testing frameworks", () => {
      const extractor = new TagExtractor({ defaultTags: [] });
      const result = extractor.extractTags("Write tests with Jest and React Testing Library");
      expect(result.ok).toBe(true);
      if (result.ok) {
    ...
    test("respects minConfidence threshold", () => {
      const extractor = new TagExtractor({ defaultTags: [], minConfidence: 0.5 });
      const result = extractor.extractTags("react");
      expect(result.ok).toBe(true);
    });
  });
  ...
  describe("extractTagsForType", () => {
    test("extracts tags for specific memory type with tag lists", () => {
      const extractor = new TagExtractor({
        defaultTags: [],
        memoryTypeTags: {
          decision: [{ id: "tech", description: "Tech stack", tags: ["react", "python"], manageable: true }],
        },
      });

      const result = extractor.extractTagsForType("We use React for frontend and Python for backend", "decision");
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.tags).toContain("React");
        expect(result.tags).toContain("Python");
    ...
    test("filters tags not in memory type tag list", () => {
      const extractor = new TagExtractor({
        defaultTags: [],
        memoryTypeTags: {
          decision: [{ id: "tech", description: "Tech", tags: ["python"], manageable: false }],
        },
      });

      const result = extractor.extractTagsForType("React and Python", "decision");
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.tags).toContain("Python");
    ...
    test("includes manageable tags from config", () => {
      const extractor = new TagExtractor({
        defaultTags: [],
        memoryTypeTags: {
          decision: [{ id: "manage", description: "Manageable", tags: ["rust"], manageable: true }],
        },
      });

      const result = extractor.extractTagsForType("Working with rust", "decision");
      expect(result.ok).toBe(true);
    });
  });
  ...
  describe("getAvailableTags", () => {
    ...
    test("returns custom tags", () => {
      const extractor = new TagExtractor({ defaultTags: [], customTags: ["custom"] });
      const tags = extractor.getAvailableTags();
    ...
    test("returns memory type tags", () => {
      const extractor = new TagExtractor({
        defaultTags: [],
        memoryTypeTags: {
          decision: [{ id: "t1", description: "t", tags: ["python"], manageable: false }],
        },
      });
    ...
    test("returns combined tags", () => {
      const extractor = new TagExtractor({
        ...
        customTags: ["custom-tag"],
        memoryTypeTags: {
          decision: [{ id: "t1", description: "t", tags: ["memory-tag"], manageable: false }],
        },
      });
      const tags = extractor.getAvailableTags("decision");
  ...
  describe("helper methods", () => {
    ...
    test("getTagListsForType returns tag lists", () => {
      const tagLists = [{ id: "t1", description: "test", tags: ["a", "b"], manageable: true }];
      const extractor = new TagExtractor({
        defaultTags: [],
  ...
  describe("heuristics", () => {
    test("disables heuristics when configured", () => {
      const extractor = new TagExtractor({ defaultTags: [], enableHeuristics: false });
...
describe("discriminated union result", () => {
  test("ExtractionResult has ok: true", () => {
    const result: TagExtractionResult = { ok: true, tags: ["React"], confidence: { React: 0.8 } };
    expect(result.ok).toBe(true);
  });

  test("ExtractionError has ok: false", () => {
    const result: TagExtractionResult = { ok: false, error: "test", code: "empty_content" };
    expect(result.ok).toBe(false);
  });
