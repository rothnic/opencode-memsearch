...
const isLifecycleSuccess = <T>(result: LifecycleResult<T>): result is { ok: true; data: T } => {
  return result.ok === true;
};

const isLifecycleError = <T>(result: LifecycleResult<T>): result is { ok: false; error: CollectionLifecycleError } => {
...
const createMockShell = (responses: Record<string, { stdout: string; stderr: string; exitCode: number }>) => {
  const mockFn = vi.fn((strings: TemplateStringsArray, ...values: unknown[]) => {
    ...
    const result = { exitCode: response.exitCode, stdout: response.stdout, stderr: response.stderr };
    return {
...
describe("CollectionLifecycle Factory", () => {
  test("fromConfig creates instance with workdir", () => {
    ...
    expect(lifecycle.getTrackingFilePath()).toBe("/tmp/test/.memsearch/collections.json");
  });

  test("fromConfig accepts custom shell", () => {
    const mockShell = createMockShell({});
    const lifecycle = CollectionLifecycle.fromConfig({ workdir: "/tmp/test", shell: mockShell });
    expect(lifecycle).toBeDefined();
  });
});
...
describe("trackCollection", () => {
  ...
  beforeEach(async () => {
    ...
    await rm(join(testRoot, ".memsearch", "collections.json"), { force: true }).catch(() => {});
  });

  test("tracks new collection with metadata", async () => {
    const lifecycle = CollectionLifecycle.fromConfig({ workdir: testRoot });
    const result = await lifecycle.trackCollection("test-collection", { documentCount: 10, memoryType: "context" });
  ...
  test("updates existing collection metadata", async () => {
    ...
    const result = await lifecycle.trackCollection("existing", { documentCount: 15 });
    expect(isLifecycleSuccess(result)).toBe(true);
...
describe("getTrackedCollection", () => {
  ...
  test("returns tracked collection data", async () => {
    const lifecycle = CollectionLifecycle.fromConfig({ workdir: testRoot });
    await lifecycle.trackCollection("my-collection", { documentCount: 25, memoryType: "decision" });
...
describe("deleteCollection", () => {
  ...
  test("fails for untracked collection", async () => {
    const mockShell = createMockShell({});
    const lifecycle = CollectionLifecycle.fromConfig({ workdir: testRoot, shell: mockShell });
  ...
  test("calls memsearch delete command", async () => {
    const mockShell = createMockShell({
      'sh -c "memsearch delete --collection test-collection"': { stdout: "", stderr: "", exitCode: 0 },
    });
    const lifecycle = CollectionLifecycle.fromConfig({ workdir: testRoot, shell: mockShell });
    await lifecycle.trackCollection("test-collection");
    const result = await lifecycle.deleteCollection("test-collection", { untrack: true });
...
describe("getStatus", () => {
  ...
  test("returns status with tracked collections", async () => {
    const mockShell = createMockShell({
      'sh -c "memsearch stats --collection existing"': { stdout: '{"documentCount": 100}', stderr: "", exitCode: 0 },
    });
    const lifecycle = CollectionLifecycle.fromConfig({ workdir: testRoot, shell: mockShell });
...
describe("syncWithMemsearch", () => {
  ...
  test("removes collections that no longer exist in memsearch", async () => {
    const mockShell = createMockShell({
      'sh -c "memsearch stats --collection ghost-collection"': { stdout: "", stderr: "collection not found", exitCode: 1 },
    });
    const lifecycle = CollectionLifecycle.fromConfig({ workdir: testRoot, shell: mockShell });
  ...
  test("keeps collections that exist in memsearch", async () => {
    const mockShell = createMockShell({
      'sh -c "memsearch stats --collection valid-collection"': { stdout: '{"documentCount": 10}', stderr: "", exitCode: 0 },
    });
    const lifecycle = CollectionLifecycle.fromConfig({ workdir: testRoot, shell: mockShell });
