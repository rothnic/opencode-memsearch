...
export class CollectionLifecycle {
  ...
  private constructor(config: CollectionLifecycleConfig) {
    ...
    this.trackingFilePath = join(this.workdir, ".memsearch", "collections.json");
   ...
   */
  async getTrackedCollection(name: string): Promise<LifecycleResult<TrackedCollection | null>> {
    try {
      const data = await this.loadTrackingData();
  ...
  async deleteCollection(
    ...
    try {
      ...
      const deleteResult = await this.shell`sh -c "memsearch delete --collection ${name}"`.throws();
      ...
      if (errorMessage.includes("collection not found") || errorMessage.includes("not exist")) {
        if (untrack) {
          await this.untrackCollection(name);
        }
        return { ok: true, data: true };
  ...
  async getStatus(): Promise<LifecycleResult<LifecycleStatusReport>> {
    try {
      ...
      for (const tracked of Object.values(data.collections)) {
        ...
        try {
          const statsResult = await this.shell`sh -c "memsearch stats --collection ${tracked.name}"`.quiet();
          if (statsResult.exitCode === 0) {
            ...
            try {
              ...
              const match = (statsResult.stdout ?? "").match(/Document[s]?:\s*(\d+)/i);
              if (match) {
