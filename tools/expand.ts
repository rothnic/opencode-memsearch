...
import type { ExpandResult, MemsearchToolContext } from "../lib/types/index";

export const memExpandTool = tool({
	description:
		"Expand a chunk_hash into full context: source, heading, and full content",
	args: {
	...
	async execute(args, _context) {
		...
		try {
			...
			const expanded = results.map((r) => ({
				...
				markdown:
					`### Source: ${r.source?.uri ?? r.source?.name ?? "unknown"}\n` +
