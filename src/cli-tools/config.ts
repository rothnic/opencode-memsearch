...
import type { MemsearchConfig, MemsearchToolContext } from "../types/index";
...
export const memConfigTool = tool({
	...
	args: {
		...
		key: tool.schema
			.string()
			.optional()
			.describe("Configuration key (required for set, optional for get)"),
		value: tool.schema
			.string()
			.optional()
			.describe("Value to set (required for set)"),
	},

	async execute(rawArgs, _context) {
		const context = _context as MemsearchToolContext;
		const { action, key, value } = rawArgs as {
			action: "get" | "set";
			key?: string;
			value?: string;
		};
		...
		try {
			...
			if (!key || value === undefined) {
				return JSON.stringify({
					ok: false,
					message: "key and value are required for action 'set'",
				});
			}
