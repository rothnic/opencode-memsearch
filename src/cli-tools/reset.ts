...
export const memResetTool = tool({
	description:
		"Reset (drop) the memsearch index. Requires explicit confirmation",
	args: {
		confirm: tool.schema
			.boolean()
			.describe("Must be true to drop indexed data (acts like --yes)"),
	},
	...
	async execute(rawArgs, _context) {
		...
		if (!confirm) {
			const out = {
				ok: false,
				error:
					"Confirmation required. Pass confirm=true to drop indexed data. This is destructive.",
			};
