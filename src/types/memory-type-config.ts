...
export const MemoryTypeConfigSchema = z.object({
	...
	collection: z.string().refine((s) => CollectionNameRegex.test(s), {
		message: "collection must match /^[a-zA-Z][a-zA-Z0-9_-]{2,63}$/",
	}),
	extractionPrompt: z
		.string()
		.min(MIN_PROMPT_LENGTH, {
			message: `extractionPrompt must be at least ${MIN_PROMPT_LENGTH} chars`,
		})
		.max(MAX_PROMPT_LENGTH, {
			message: `extractionPrompt must be at most ${MAX_PROMPT_LENGTH} chars`,
		}),
	model: z
		.string()
		.optional()
	...
	output: MemoryConfigOutputSchema.optional().default({
		path: "memory",
