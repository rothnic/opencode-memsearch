...
export const memSearchTool = tool({
	description:
		"Search the memsearch index and return formatted results (includes chunk_hash)",
	args: {
		...
		minScore: tool.schema
			.number()
			.optional()
			.describe("Minimum score threshold (0..1)"),
		smart: tool.schema
			.boolean()
			.optional()
			.describe("Use smart search (semantic rerank / expansion)"),
		filter: tool.schema
			.string()
			.optional()
			.describe("Metadata filter expression (e.g. 'source == \"file.txt\"')"),
	},
	...
	async execute(rawArgs, _context) {
		...
		try {
			...
			if (smart !== undefined)
				options.smart = smart ? { enabled: true } : false;
