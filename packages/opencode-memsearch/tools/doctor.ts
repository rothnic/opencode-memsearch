		...
		try {
			...
			if (available) {
				checks.push({
					name: "memsearch_cli",
					ok: true,
					detail: "memsearch CLI found in PATH",
				});
			} else {
				checks.push({
					name: "memsearch_cli",
			...
			checks.push({
				...
				ok: false,
				detail: String(err),
				fix: "Install with: pip install memsearch",
			});
		}
			...
			if (
				...
				if (key && key.length > 0) {
					checks.push({
						...
						ok: true,
						detail: `Provider='${provider}' and embeddingApiKey is set`,
					});
				} else {
					checks.push({
						name: "embedding_api_key",
				...
				checks.push({
					name: "embedding_api_key",
					ok: true,
					detail: `Provider='${provider}' does not require embeddingApiKey`,
				});
			}
		} catch (err: any) {
			checks.push({
				name: "embedding_api_key",
				ok: false,
				detail: `failed to load config: ${String(err)}`,
			});
		}
		...
		try {
			...
			if (writable) {
				checks.push({
					...
					ok: true,
					detail: `Memory directory '${memdir}' is writable`,
				});
			} else {
				checks.push({
					name: "memory_directory_writable",
			...
			checks.push({
				name: "memory_directory_writable",
				ok: false,
				detail: `error checking memoryDirectory: ${String(err)}`,
			});
		}
		...
		try {
			try {
				...
				const found =
					res.exitCode === 0 &&
					res.stdout &&
					String(res.stdout).trim().length > 0;
				if (found) {
					checks.push({
						name: "pip_package",
						ok: true,
						detail: "pip package 'memsearch' is installed",
					});
				} else {
					checks.push({
						name: "pip_package",
						ok: false,
						detail: "pip show memsearch returned no data",
						fix: "Install with: pip install memsearch",
					});
				}
			} catch (err: any) {
				checks.push({
					name: "pip_package",
					ok: false,
					detail: `pip show failed: ${String(err)}`,
					fix: "Ensure pip is available and memsearch is installed (pip install memsearch)",
				});
			}
		} catch (err: any) {
			checks.push({
				name: "pip_package",
				ok: false,
				detail: `failed to run pip show: ${String(err)}`,
			});
		}
