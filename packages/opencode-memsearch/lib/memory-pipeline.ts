...
async function withRetry<T>(
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      console.log(`[memsearch] ${context}: ${operationName} (attempt ${attempt}/${MAX_RETRIES})`);
      ...
      if (!isRetryable) {
        console.error(`[memsearch] ${context}: Non-retryable error - ${errorMsg}`);
        throw err;
      }
      
      if (attempt < MAX_RETRIES) {
        console.warn(`[memsearch] ${context}: Service unavailable, retrying in ${RETRY_DELAY_MS}ms...`);
        console.warn(`[memsearch] ${context}: Error: ${errorMsg}`);
        await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS));
      } else {
        console.error(`[memsearch] ${context}: All ${MAX_RETRIES} attempts failed`);
...
async function processSessionCreated(job: MemoryJob): Promise<ProcessResult> {
  ...
  if (!isAvailable) {
    console.error(`[memsearch] ${projectId}: Memsearch CLI not available. Please install: pip install memsearch`);
  ...
  try {
    ...
    if (errorMsg.includes('ollama') || errorMsg.includes('Ollama')) {
      console.error(`[memsearch] ${projectId}: Ollama embedding service issue. Check:`);
      console.error(`[memsearch] ${projectId}:   - Ollama is running: ollama serve`);
      console.error(`[memsearch] ${projectId}:   - Config has correct host in .memsearch.toml`);
      console.error(`[memsearch] ${projectId}:   - Model is pulled: ollama pull nomic-embed-text`);
    }
    
    if (errorMsg.includes('milvus') || errorMsg.includes('Milvus')) {
      console.error(`[memsearch] ${projectId}: Milvus vector database issue. Check:`);
      console.error(`[memsearch] ${projectId}:   - Milvus is running`);
      console.error(`[memsearch] ${projectId}:   - Connection URI in .memsearch.toml`);
    }
...
async function processManualIndex(job: MemoryJob): Promise<ProcessResult> {
  ...
  try {
    ...
    const errorMsg = String(err);
    console.error(`[memsearch] ${projectId}: Manual indexing failed: ${errorMsg}`);
    return { success: false, error: errorMsg, retryable: true };
  }
}
