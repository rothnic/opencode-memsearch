...
`;
	...
	afterAll(async () => {
		await rm(join(env.home, ".memsearch"), {
			recursive: true,
			force: true,
		}).catch(() => {});
		await env.cleanup();
	...
	it(
		"loads plugin and memsearch CLI is available",
		async () => {
	...
	it(
		"indexes real files into named collections and returns stats",
		async () => {
			...
			const idx1 = await tools["mem-index"].execute(
				{ path: file1, collection: "e2e_coll_animals" },
				mockContext,
			);
			const idx2 = await tools["mem-index"].execute(
				{ path: file2, collection: "e2e_coll_fruit" },
				mockContext,
			);
			...
			expect(
				parsed1.stats.documentCount + parsed2.stats.documentCount,
			).toBeGreaterThan(0);
		},
		TIMEOUT,
	);
	...
	it(
		"performs real search against indexed collections",
		async () => {
			const rawSearch = await tools["mem-search"].execute(
				{ query: "bananas", collection: "e2e_coll_fruit", topK: 5 },
				mockContext,
			);
			...
			expect(
				r.preview.toLowerCase().includes("banana") ||
					r.preview.toLowerCase().includes("bananas"),
			).toBe(true);
		},
		TIMEOUT,
	);
	...
	it(
		"supports multi-source context injection via real collections",
		async () => {
			...
			await writeFile(
				join(env.home, "worktree", "opencode.json"),
	...
	it(
		"supports search options (minScore, topK) producing filtered results",
		async () => {
			const rawTopK = await tools["mem-search"].execute(
				{ query: "bananas", topK: 1 },
				mockContext,
			);
			const respTopK = JSON.parse(rawTopK as string);
			expect(respTopK.count).toBeLessThanOrEqual(1);

			const rawMin = await tools["mem-search"].execute(
				{ query: "bananas", minScore: 0.99 },
				mockContext,
			);
