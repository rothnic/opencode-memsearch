...
```bash
npm install opencode-memsearch
```
...
For development, reference the local path:

```json
{
  "plugin": ["file:///path/to/opencode-memsearch"]
}
```
...
If you find inconsistencies or missing behavior, inspect the plugin source under `packages/opencode-memsearch/` for implementation details and open an issue with relevant logs.

## Developer notes

- Tools live under `packages/opencode-memsearch/tools/` and are thin wrappers around `cli-wrapper.ts` which invokes the `memsearch` binary using Bun's $`` helper.
- The configuration loader `config.ts` merges `opencode.json` -> `memsearch` with safe defaults and uses Zod for validation.
- Hooks are registered in `index.ts` and live under `packages/opencode-memsearch/hooks/`.
