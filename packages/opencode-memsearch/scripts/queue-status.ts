...
function displayStatus(
	...
	if (jobs.length > 0) {
		...
		console.log("─".repeat(80));
		console.log(
			`${"State".padEnd(12)} ${"Project".padEnd(20)} ${"Created".padEnd(20)} ${"Duration"}`,
		);
		console.log("─".repeat(80));
		...
		for (const job of jobs) {
			...
			const project =
				projectId.length > 18 ? projectId.slice(0, 18) + ".." : projectId;
			...
			console.log(
				`${state} ${project.padEnd(20)} ${created.padEnd(20)} ${duration}`,
			);
		}
		console.log();
	}
