...
export class MemsearchCLI {
	...
	async checkAvailability(): Promise<boolean> {
		...
		try {
			const result = await this
				.shell`sh -c "python3 -m memsearch --version"`.quiet();
	...
	async index(
		path: string,
		options: { recursive?: boolean; collection?: string } = {},
	): Promise<void> {
	...
	async compact(): Promise<string> {
		...
		const output = await this
			.shell`sh -c "python3 -m memsearch compact"`.text();
		return output;
	}
	...
	async expand(query: string): Promise<ExpandResult[]> {
		await this.ensureAvailable();
		const output = await this
			.shell`sh -c "python3 -m memsearch expand ${query} --json-output"`.text();
	...
	async transcript(sessionId: string): Promise<TranscriptEntry[]> {
		await this.ensureAvailable();
		const output = await this
			.shell`sh -c "python3 -m memsearch transcript ${sessionId} --json-output"`.text();
	...
	async config(
		...
		if (action === "get") {
			...
			await this
				.shell`sh -c "python3 -m memsearch config set ${key} ${value}"`.throws(
				true,
			);
		}
	}
	...
	async stats(): Promise<MemsearchStats> {
		await this.ensureAvailable();
		const output = await this
			.shell`sh -c "python3 -m memsearch stats --json-output"`.text();
	...
	async version(): Promise<string> {
		...
		try {
			const output = await this
				.shell`sh -c "python3 -m memsearch --version"`.text();
			return output.trim();
		} catch (err) {
