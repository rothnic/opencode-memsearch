...
export async function loadConfig(workdir: string): Promise<MemsearchConfig> {
	...
	const merged = mergeWithLegacyConfig(yamlConfig, legacyConfig) as Record<
		string,
		unknown
	>;
	...
	const extras: Record<string, unknown> = {
		...(merged.extras as Record<string, unknown>),
	};
	...
	const hasLegacy =
		existsSync(legacyConfigPath) && legacyConfigHasMemsearch(legacyConfigPath);
	...
	if (hasLegacy && !hasYaml) {
		...
		const migrationWarning: MigrationWarning = {
			...
			instruction:
				"Run: cp .memsearch.yaml.example .memsearch.yaml (or manually create .memsearch.yaml based on your opencode.json memsearch section)",
		...
		const migrationInfo = {
			type: "migrate_to_yaml",
			message:
				"Both opencode.json and .memsearch.yaml present - YAML takes precedence",
			instruction:
				"Once migrated, you can remove the memsearch section from opencode.json",
			legacyConfigPresent: true,
	...
	for (const userSource of userSources) {
		...
		if (index !== -1) {
			merged[index] = {
				...
				injection: {
					...merged[index].injection,
					...(userSource.injection || {}),
				},
			};
		} else {
